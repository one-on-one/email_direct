...
class TestPublicationFacade < Test::Unit::TestCase
  ...
  def setup()
    ...
    @publication_get_all1 = {:name => "name#{uuid2}", :description => "description#{uuid2}"}

    uuid3 = generator.generate(:compact)
    @publication_get_all2 = {:name => "name#{uuid3}", :description => "description#{uuid3}"}

    uuid4 = generator.generate(:compact)
    @publication_get_all3 = {:name => "name#{uuid4}", :description => "description#{uuid4}"}
  ...
  # Tests for One::EmailDirect::Facade.publication_get_all.
  #
  # 1. no publications
  # 2. one publication
  # 3. two publications
  ...
  #
  def test_publication_get_all()
    # 1. no publications
    assert_equal [], One::EmailDirect::Facade.publication_get_all(@credentials)


    # 2. one publication
    One::EmailDirect::Facade.publication_add(@credentials, @publication_get_all1[:name], @publication_get_all1[:description])
    result = One::EmailDirect::Facade.publication_get_all(@credentials)
    expected = [{
        :element_name => @publication_get_all1[:name],
        :description => @publication_get_all1[:description]
    }]
    ...
    # 3. two publications
    One::EmailDirect::Facade.publication_add(@credentials, @publication_get_all2[:name], @publication_get_all2[:description])
    result = One::EmailDirect::Facade.publication_get_all(@credentials)
    expected = [{
        :element_name => @publication_get_all1[:name],
        :description => @publication_get_all1[:description]
    }, {
        :element_name => @publication_get_all2[:name],
        :description => @publication_get_all2[:description]
    }]
    result.collect! {|element| element.delete(:element_id); element} # remove ids
    assert_equal expected, result
    ...
    # 4. three publications
    One::EmailDirect::Facade.publication_add(@credentials, @publication_get_all3[:name], @publication_get_all3[:description])
    result = One::EmailDirect::Facade.publication_get_all(@credentials)
    expected = [{
        :element_name => @publication_get_all1[:name],
        :description => @publication_get_all1[:description]
        ...
        :element_name => @publication_get_all3[:name],
        :description => @publication_get_all3[:description]
    }]
    result.collect! {|element| element.delete(:element_id); element} # remove ids
    assert_equal expected, result
  end
